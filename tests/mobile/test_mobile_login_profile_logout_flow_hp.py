...
@pytest.mark.mobile
def test_mobile_login_profile_logout_flow(): # Keep the original function name for the _hp.py variant
    """
    ...
    try:
        with sync_playwright() as p:
            ...
            # *** CORRECTED LOCATOR for "Download on the App Store" link ***
            # Use get_by_role with name and then .first to resolve ambiguity
            app_store_link_locator = page.get_by_role("link", name="Download on the App Store").first
            
            expect(app_store_link_locator).to_be_visible(timeout=15000)
            logger.info("Verified 'Download on the App Store' link is visible, confirming logged-out state.")

            page.screenshot(path=test_report_dir / "final_08_after_logout_verified_homepage_app_store.png")
            logger.info("Successfully logged out and verified return to the main homepage.")
